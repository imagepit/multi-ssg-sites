...
const buildDependencies = (rootDir: string) => {
  ...
  const deploySite = new DeploySiteUseCase(
    runner,
    logger,
    fileSystem,
    paths,
...
const parseSiteId = (value: string): SiteId => SiteId.create(value)
const parseThemeId = (value: string): ThemeId => ThemeId.create(value)
const parsePort = (value: string): Port => Port.create(value)

const resolveSearchIndexBaseUrl = (siteId: SiteId, override?: string): string | undefined => {
