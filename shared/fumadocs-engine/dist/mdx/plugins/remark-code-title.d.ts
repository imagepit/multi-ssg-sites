import type { Root, Paragraph, Code, PhrasingContent } from 'mdast'
...
export function remarkCodeTitleBeforeBlocks() {
  return (tree: Root) => {
    visit(tree, 'paragraph', (node, index, parent) => {
      ...
      const next = parent.children[index + 1] as Code | undefined
...
function extractEmphasisTitle(p: Paragraph): string | null {
  if (!p.children || p.children.length !== 1) return null
  const child = p.children[0] as PhrasingContent & { children?: PhrasingContent[] }
  if (child.type !== 'emphasis' && child.type !== 'strong') return null
...
function flattenPhrasing(n: any): string {
  if (!n) return ''
  if (typeof n.value === 'string') return n.value
  if (Array.isArray(n.children)) return n.children.map((c: any) => flattenPhrasing(c)).join('')
  return ''
}
