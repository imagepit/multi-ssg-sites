...
export function remarkAddDelRegions() {
    return (tree) => {
        visit(tree, 'code', (node) => {
            ...
            const is = (re, s) => re.test(s);
            for (const line of lines) {
                const trimmed = line.trim();
                // Region boundaries (case-insensitive)
                if (is(/^\/\/\s*addstart\s*$/i, trimmed)) {
                    setMode('add');
                    continue;
                }
                if (is(/^\/\/\s*addend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
                if (is(/^\/\/\s*delstart\s*$/i, trimmed)) {
                    setMode('del');
                    continue;
                }
                if (is(/^\/\/\s*delend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
                if (is(/^\/\/\s*highlightstart\s*$/i, trimmed)) {
                    setMode('highlight');
                    continue;
                }
                if (is(/^\/\/\s*highlightend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
                if (is(/^\/\/\s*focusstart\s*$/i, trimmed)) {
                    setMode('focus');
                    continue;
                }
                if (is(/^\/\/\s*focusend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
                if (is(/^\/\/\s*errorstart\s*$/i, trimmed)) {
                    setMode('error');
                    continue;
                }
                if (is(/^\/\/\s*errorend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
                if (is(/^\/\/\s*warningstart\s*$/i, trimmed)) {
                    setMode('warning');
                    continue;
                }
                if (is(/^\/\/\s*warningend\s*$/i, trimmed)) {
                    setMode(null);
                    continue;
                }
