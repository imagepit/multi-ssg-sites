...
function parseHsl(input: string): HSL | null {
  // supports: hsl(266deg 85% 58%) or hsl(266,85%,58%)
...
function parseAnyToHsl(input: string): HSL | null {
  return (
    parseHsl(input) ||
    (() => {
...
function toHsl({ h, s, l }: HSL): string {
  const hh = ((h % 360) + 360) % 360
  return `hsl(${hh}deg ${clamp(s, 0, 100)}% ${clamp(l, 0, 100)}%)`
}

function adjust(hsl: HSL, ds = 0, dl = 0): HSL {
  return { h: hsl.h, s: clamp(hsl.s + ds, 0, 100), l: clamp(hsl.l + dl, 0, 100) }
}

function chooseForeground(lightness: number): string {
  // Simple contrast heuristic
  return lightness < 55 ? 'hsl(0 0% 98%)' : 'hsl(0 0% 9%)'
}

export function deriveThemeFromPrimary(primaryRaw: string | null | undefined) {
  ...
  // Dark variants: keep brand hue, increase brightness, reduce saturation slightly
  const darkBase: HSL = { h: base.h, s: clamp(base.s * 0.7, 20, 90), l: clamp(92 - (100 - base.l) * 0.3, 60, 96) }
...
export function buildThemeCssFromPrimary(primary: string | null | undefined): string {
