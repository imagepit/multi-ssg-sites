...
export function rehypeResolveImages(options?: { imagesDir?: string }) {
  const imagesDir = options?.imagesDir || path.join(process.cwd(), 'public', 'images')
  return function transformer(tree: any, file: any) {
    const filePath: string | undefined = typeof file?.path === 'string' ? file.path : undefined
    ...
    visit(tree, 'element', (node: Element) => {
      ...
      if (hintIdx >= 0) { node.properties.src = '/' + rawSrc.replace(/\\/g, '/').slice(hintIdx); return }
